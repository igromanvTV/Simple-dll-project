...
typedef struct Proto
{
    ...
    LUAU_SHUFFLE4(LUAU_SEMICOLON_SEP,
        proto_k<TValue*> k,              // constants used by the function
        proto_code<Instruction*> code,      // function bytecode
        proto_p<struct Proto**> p,       // functions defined inside the function
    ...
    LUAU_SHUFFLE5(LUAU_SEMICOLON_SEP,
        proto_lineinfo<uint8_t*> lineinfo,      // for each instruction, line number as a delta from baseline
        proto_abslineinfo<int*> abslineinfo,       // baseline line info, one entry for each 1<<linegaplog2 instructions; allocated after lineinfo
        proto_locvars<struct LocVar*> locvars, // information about local variables
        proto_upvalues<TString**> upvalues,     // upvalue names
        proto_source<TString*> source);

    proto_debugname<TString*> debugname;
    proto_debuginsn<uint8_t*> debuginsn; // a copy of code[] array with just opcodes
